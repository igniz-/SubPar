...
{
    qDebug() << "DownloadManager::execute";
...
void DownloadManager::doDownload(const QUrl &url) {
    qDebug() << "DownloadManager::doDownload";

    QNetworkRequest request(url);
    QNetworkReply *reply = manager.get(request);

#ifndef QT_NO_SSL
    qDebug() << "connect sslErrors";
    connect(reply, SIGNAL(sslErrors(QList<QSslError>)), SLOT(sslErrors(QList<QSslError>)));
#endif

    // List of reply
    currentDownloads.append(reply);
    qDebug() << "reply added to queue";
}

QString DownloadManager::saveFileName(const QUrl &url) {
    qDebug() << "DownloadManager::saveFileName";
...
void DownloadManager::downloadFinished(QNetworkReply *reply) {
    qDebug() << "DownloadManager::downloadFinished";
    QUrl url = reply->url();
    if (reply->error()) {
...
bool DownloadManager::saveToDisk(const QString &filename, QIODevice *reply) {
    qDebug() << "DownloadManager::saveToDisk";
    QFile file(filename);
